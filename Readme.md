...
*   **F-UI-03:** 状态栏应实时显示以下信息：GNSS锁定卫星数量、SD卡状态（是否存在）、电池电量百分比、充电状态图标。
*   ***F-UI-04:** 为提高代码可维护性，每个主功能界面的UI代码应被组织在独立的源文件中（例如 `ui_bike_computer.cpp`, `ui_pbox.cpp`）。

#### 用户输入 (INPUT)
*   **F-INPUT-01:** 系统应能精确响应旋转编码器的旋转、短按、中按、长按四种事件。
...
#### 自行车码表 (BC)
*   **F-BC-01:** 自行车码表界面必须实时显示以下核心数据：当前速度（km/h）、海拔高度（m）、本次骑行累计里程（km）。

#### GPS轨迹记录 (LOG)
*   **F-LOG-01:** GPS轨迹记录功能应将轨迹数据以GPX格式写入SD卡的 `/GPX/` 目录中。GPX文件必须支持包含温度、G值等数据的扩展标签。

#### 汽车性能测试盒 (PBOX)
*   **F-PBOX-01:** 汽车性能测试盒模式应提供0-100km/h的加速测试功能。
